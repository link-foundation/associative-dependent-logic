...
| `E039` | Tactic, rewrite engine, SMT bridge, or ATP bridge error. Triggered when a link tactic is malformed, unknown, applied to an incompatible goal, cannot close/transform the current goal, a simplification rewrite set reaches its termination guard, `(by smt)` cannot prove a goal because the solver is unconfigured, times out, returns `sat`/`unknown`, or fails to run, or `(by atp)` cannot export, invoke, or parse a configured ATP result. The message includes the current goal printed as a link for tactic failures. |
