...
| `E039` | Tactic, rewrite engine, or ATP bridge error. Triggered when a link tactic is malformed, unknown, applied to an incompatible goal, cannot close/transform the current goal, a simplification rewrite set reaches its termination guard, or `(by atp)` cannot export/invoke/parse a configured ATP result. The message includes the current goal printed as a link for tactic failures. |
