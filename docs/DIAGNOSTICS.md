...
| `E039` | Tactic and rewrite engine error. Triggered when a link tactic is malformed, unknown, applied to an incompatible goal, cannot close/transform the current goal, a simplification rewrite set reaches its termination guard, or the SMT tactic cannot prove a goal because the solver is unconfigured, times out, returns `sat`/`unknown`, or fails to run. The message includes the current goal printed as a link for tactic failures. |
